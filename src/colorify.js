...
(function(exports) {
  ...
  function validateOptions(passed, booleans, strings, functions) {
    ...
    boolList.forEach(function(field) {
      newOptions[field] = isSet(passed[field])
        ? !!passed[field]
        : booleans[field];
    });
    ...
    stringList.forEach(function(field) {
      newOptions[field] = isString(passed[field])
        ? passed[field]
        : strings[field];
    });
    ...
    fnList.forEach(function(field) {
      newOptions[field] = isFunction(passed[field])
        ? passed[field]
        : functions[field];
    });
  ...
  function getErrors(errors) {
    ...
    if (errors.length) {
      ...
      error = new Error(
        'Error loading the following images: ' + list.join(',')
      );
    }
    return error;
  }
  ...
  function makeMask(images, opts, callback) {
    ...
    opts.strategy =
      isString(opts.strategy) && opts.strategy in avgFn
        ? avgFn[opts.strategy]
        : opts.strategy;
  ...
  function computeMask(original, maker) {
    ...
    for (i = 0; i < data.length; i += 4) {
      ...
      if (data[i + 3]) {
        data[i] = data[i + 1] = data[i + 2] = avgFn(
          data[i],
          data[i + 1],
          data[i + 2]
        );
